...
### Mermaid対応 ✅ 完了 (2026-04-22)

- [x] `mermaid@^11.0.0` は package.json に含まれインストール済み
- [x] `src/components/cards/MermaidRenderer.tsx` を作成
  - [x] `useEffect` + `mermaid.render()` でクライアントサイドのみで SVG 生成
  - [x] エラーハンドリング・ローディング UI 実装
- [x] CardPreview の react-markdown に `components.code` カスタマイズで MermaidRenderer を統合
  - [x] `next/dynamic({ ssr: false })` でバンドル分割・SSR 無効化

### ダッシュボードへの動線

- [x] `src/app/(app)/dashboard/page.tsx` の最小実装 (今日の復習カード枚数表示のみ) (2026-04-23)
  - 復習セッション本体はM3で実装するが、ナビゲーション起点としてページを作る
- [x] AppNav にデッキ一覧へのリンクを追加 (2026-04-21)
- [x] AppNav にダッシュボードへのリンクを追加 (2026-04-23)
...
- [x] Mermaidダイアグラムがカードプレビューで描画される
